...
links <- readRDS('.build/captured/assertion-links.rds')
link_index <- 0L
rows <- list()
id <- 0L
for (file in names(x)) for (test in x[[file]]) {
  ...
  for (i in seq_along(test$results)) {
    ...
    rows[[length(rows)+1L]] <- data.frame(test_id=id,file=file,test=test$test,
      assertion=i,call_id=link$call_id,line=if(length(pos))pos[1] else NA,
...
stopifnot(link_index==length(links))
