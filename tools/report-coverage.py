...
for test in inventory['tests']:
    ...
    if exported:
        ...
        test['status'] = 'pending-native-generator'
        test['stata_tests'] = []
        coverage = 'R baseline only'
    if generator:
        test['native_generator'] = 'pending'
        pending += 1
        coverage += '; native generator pending'
...
The native random-data generator is pending. {pending} original test cases
...
''')
print(f'Updated coverage for {len(rows)} R tests; {pending} include pending native generator scope.')
